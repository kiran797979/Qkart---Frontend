...
const Products = () => {
   ...
   * }
   */
  const performAPICall = async (append = false, skip = 0) => {
    try{
      ...
      if (append) {
        updateProduct(prev => [...prev, ...product.data]);
      } else {
        updateProduct(product.data);
      }
      if (product.data.length < limit) setHasMore(false);
  ...
  useEffect( ()=>{
    async function onLoad(){
       setPage(0);
       setHasMore(true);
       const product=await performAPICall(false, 0);
  ...
  const loadMoreProducts = async () => {
    ...
    await performAPICall(true, skip);
  };
                    ...
                    <Grid container spacing={{ xs: 2, md: 3 ,lg:1 }} >
                      ...
                      {hasMore && (
                        <Grid item xs={12} style={{ textAlign: 'center', marginTop: 16 }}>
                          <button onClick={loadMoreProducts} disabled={isLoadingMore} style={{padding: '8px 24px', fontSize: '1rem'}}>
